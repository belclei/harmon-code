...
export interface MonoProps extends HTMLAttributes<HTMLElement> {
  /** "number" is for money/quantities — tabular-nums via the `.lr-money` primitive (IMPLEMENTACAO §7: "Números/valores em Red Hat Mono com tabular-nums, nunca formatar dinheiro à mão"). "code" adds a subtle inline chip for code snippets. */
  variant?: MonoVariant;
...
const TONE_CLASSES: Record<MonoTone, string> = {
  default: "lr-money",
  in: "lr-money lr-money--in",
  out: "lr-money lr-money--out",
  estimate: "lr-money lr-estimate",
};
  ...
  if (variant === "code") {
    return (
      <code
        {...rest}
        className={[
          "rounded-[var(--lr-r-sm)] bg-[var(--lr-surface-sunken)] px-1.5 py-0.5 font-mono text-[.875em] text-[var(--lr-text)]",
          className,
        ].join(" ")}
      >
        {children}
