...
export function TableRow({ className = "", ...rest }: TableRowProps) {
  return (
    <tr
      {...rest}
      className={[
        "border-b border-[var(--hm-border)] transition-colors duration-150 hover:bg-[var(--hm-surface-sunken)]",
        className,
      ].join(" ")}
    />
  );
...
export function TableHeaderCell({
  ...
  return (
    <th
      {...rest}
      className={[
        "whitespace-nowrap border-b border-[var(--hm-border)] px-3 py-2.5 font-normal",
        "text-[.6875rem] tracking-[.16em] text-[var(--hm-label)] uppercase",
